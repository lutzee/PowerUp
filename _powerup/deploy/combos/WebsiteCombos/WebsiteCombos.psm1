...
function Invoke-Combo-AddVirtualDirectories($options)
{
    import-module -disablenamechecking powerupfilesystem
    import-module -disablenamechecking powerupweb
...
function ConfigureBindingDefaultOptions($options)
{
    ...
    foreach($binding in $options.bindings)
    {
        ...
        if ($binding.useselfsignedcert -eq $false)
        {
            # if binding.useselfsignedcert does not exist or is null, 
            # *or is set to false*, set false as its default value.
            $binding.useselfsignedcert = $false
        }
        else
        {
...
function SetupVirtualDirectories($options)
{
    ...
    if($options.virtualdirectories)
    {
        ...
        {
            Write-Output "Deploying virtual directory $($virtualdirectory.directoryname) to $($options.websitename)."
            ...
            if ($virtualdirectory.fullsourcepath -or $virtualdirectory.sourcefolder)
            {
                ...
                {
                    copy-mirroreddirectory $virtualdirectory.fullsourcepath $virtualdirectory.fulldestinationpath $virtualdirectory.excludedirectories
                }
            }
            ...
            {
                Write-Output "Switching virtual directory $($options.websitename)/$($virtualdirectory.directoryname) to use app pool identity for anonymous authentication."
            ...
            if ($virtualdirectory.appfabricautostart)
            {
                ...
                {
                    Write-Output "Adding AppFabric auto-start to: $($options.websitename)/$($virtualdirectory.directoryname)"
...
function RemoveVirtualDirectories($options)
{
    ...
    if($options.virtualdirectories)
    {
        foreach($virtualdirectory in $options.virtualdirectories)
        {
            Write-Output "Removing virtual directory $($virtualdirectory.directoryname) from $($options.websitename)."
...
function RemoveWebsite($options)
{
    ...
    if(Test-Path $options.fulldestinationpath) {
...
function RemoveAppFabricApplications($options)
{
    ...
    if($options.appfabricapplications)
    {
        ...
        {
            Write-Output "Removing AppFabric application from $options.websitename, virtual dir $application.virtualdirectory."
